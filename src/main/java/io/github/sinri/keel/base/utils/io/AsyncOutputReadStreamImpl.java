package io.github.sinri.keel.base.utils.io;
...
class AsyncOutputReadStreamImpl extends OutputStream implements ReadStream<Buffer>, AsyncOutputReadStream {
    ...
    /**
     * Propagate an out-of-band error (likely generated or handled by the code that feeds the output stream) to the end
     * of the
    ...
    @Override
    public AsyncOutputReadStreamImpl exceptionHandler(Handler<Throwable> handler) {
    ...
    @Override
    public AsyncOutputReadStreamImpl handler(Handler<Buffer> handler) {
    ...
    @Override
    public AsyncOutputReadStreamImpl pause() {
        paused.getAndSet(new CountDownLatch(1)).countDown();
        return this;
    }

    @Override
    public AsyncOutputReadStreamImpl resume() {
        paused.getAndSet(new CountDownLatch(0)).countDown();
        return this;
    }

    @Override
    public AsyncOutputReadStreamImpl fetch(long amount) {
        resume();
    ...
    @Override
    public AsyncOutputReadStreamImpl endHandler(Handler<Void> endHandler) {
