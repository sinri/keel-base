---
layout: default
title: keel-base 5.0.5 使用指南
---

# keel-base 5.0.5 使用指南

本文档面向使用 `keel-base` 库的开发者，系统介绍 5.0.5 版本的模块设计、核心 API 及典型使用方式。
...
```kotlin
dependencies {
    implementation("io.github.sinri:keel-base:5.0.5")
}
```
...
> `LockAcquireFailedException` 是类型化异常，方便在 `catch` 或 `onFailure` 中区分锁超时与其他错误。
...
> 5.0.5 起，`asyncTransformRawFuture` 的两个重载均已标记为废弃，并计划移除。新代码建议直接使用 Vert.x
`executeBlocking` 包装阻塞等待逻辑。

```java
java.util.concurrent.Future<String> rawFuture = executor.submit(() -> "result");

// 推荐：使用 Vert.x worker pool 执行阻塞等待
Future<String> future = keel.executeBlocking(() -> rawFuture.get());

// 如需超时，应由调用方明确表达超时语义
Future<String> futureWithTimeout = keel.executeBlocking(
        () -> rawFuture.get(5, TimeUnit.SECONDS)
);
```
...
- `asyncTransformRawFuture(rawFuture, sleepTime)`

其中 `sleepTime` 表示轮询间隔，不表示超时时间。
...
> 5.0.5 起，`blockAwait` 已标记为废弃，并计划移除。Vert.x 5 已提供原生 `Future.await()` API，新代码应优先使用 Vert.x 原生方法。

```java
// 仅可在允许阻塞的线程中使用，例如 worker 线程、虚拟线程或普通线程。
String result = someAsyncOperation().await();

// 需要超时时使用 Vert.x 原生超时重载。
String resultWithTimeout = someAsyncOperation().await(5, TimeUnit.SECONDS);
```

> 严禁在 EventLoop 线程中阻塞等待异步结果。
...
│   └── JsonObjectMappedBean     -- Bean ↔ JsonObject 双向映射（技术预览）
│
...
String missing = root.readProperty("app.missing");          // null

// 5.0.5 起：点号分隔键名的必填读取
String databaseHost = root.readPropertyRequired("database.host");

// 5.0.5 起：必填读取并集中处理默认值或类型转换
int databasePort = root.readPropertyRequired(
        "database.port",
...
> 当请求的配置项不存在时，`extract`、`readString`、`readPropertyRequired(String)` 等方法会抛出
`NotConfiguredException`。使用 `tryToExtract`、`tryToGetChild`、`readProperty` 或
`readPropertyRequired(String, Function)` 可自行处理缺失配置。
...
public class MyFileLogWriterAdapter extends FileLogWriterAdapter {
    ...
    // 5.0.5 起可按需调整队列空闲时的等待间隔，默认 100ms。
    @Override
